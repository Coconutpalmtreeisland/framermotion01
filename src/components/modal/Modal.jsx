...
const scaleAnimation = {
    ...
    enter: { scale: 1, x: "-50%", y: "-50%", transition: { duration: 0.4, ease: [0.65, 0, 0.35, 1] } },
    closed: { scale: 0, x: "-50%", y: "-50%", transition: { duration: 0.4, ease: [0.65, 0, 0.35, 1] } },
}
...
const Modal = ({ modal, texts }) => {
    ...
    useEffect(() => {
        ...
        let moveCursorTextX = gsap.quickTo(cursorText.current, "left", { duration: 0.45, ease: "power3" })
        let moveCursorTextY = gsap.quickTo(cursorText.current, "top", { duration: 0.45, ease: "power3" })
                    ...
                    {texts.map((text, index) => {
                        ...
                        return (
                            <div className='modal' key={index}>
                                <img src={`images/${src}`} alt={text.title} />
                            </div>
                        )
                    })}
                </div>
